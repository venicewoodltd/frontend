...
export default function ProjectEditForm() {
  ...
  // Helper function to extract fileId from URL
  const extractFileId = (urlOrId) => {
    if (!urlOrId) return "";
    // If it's already just an ID (24 char hex string), return it
    if (/^[a-f0-9]{24}$/i.test(urlOrId)) {
      return urlOrId;
    }
    // Extract from URL like /api/images/xxx or http://localhost:4000/api/images/xxx
    const match = urlOrId.match(/\/api\/images\/([a-f0-9]{24})/i);
    ...
    const fetchProject = async () => {
      ...
      try {
        ...
        if (data.success && data.project) {
          ...
          // Reconstruct image/galleryImages from media array
          const media = project.media || [];
          const mainMedia = media.find((m) => m.type === "main");
          const galleryMedia = media.filter((m) => m.type === "gallery");
          const imageUrl = mainMedia
            ? `${API_URL}/api/images/${mainMedia.fileId}`
            : "";
          const galleryUrls = galleryMedia.map(
            (m) => `${API_URL}/api/images/${m.fileId}`,
          );
          setFormData({
            ...
            primaryWood: project.primaryWood || "",
            client: project.client || "",
            featured: project.featured || false,
            status: project.status || "draft",
            seoTags: project.seoTags || "",
            mainImage: extractFileId(imageUrl) || "",
            galleryImages: galleryUrls.map(extractFileId),
          ...
          setError(data.error || data.message || "Failed to load project");
        }
      } catch (err) {
  ...
  const confirmRemoveGalleryImage = async () => {
    ...
    const fileIdToDelete = formData.galleryImages[index];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Add main image file ID if it's a valid GridFS ID
      if (formData.mainImage && /^[a-f0-9]{24}$/i.test(formData.mainImage)) {
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex justify-between items-start mb-8">
          <div>
            ...
            <p className="text-gray-600">Update project details and images</p>
          </div>
          <button
            type="button"
              ...
              <div className="border-2 border-dashed border-[#d7ccc8] rounded-lg p-6 text-center">
                <label className="cursor-pointer block">
                  ...
                  <div className="text-gray-500">
                    ...
                    <p className="mt-2 text-sm">Click to upload main image</p>
                  ...
                  <img
                    src={
                      imagePreview.startsWith("data:")
                        ? imagePreview
                        : `${API_URL}/api/images/${extractFileId(imagePreview)}`
                    }
                    alt="Main preview"
                  ...
                  <img
                    src={
                      preview.startsWith("data:")
                        ? preview
                        : `${API_URL}/api/images/${extractFileId(preview)}`
                    }
                    alt={`Gallery ${index + 1}`}
