...
export default function BlogEditForm() {
    ...
    try {
      ...
      if (data.success && data.blog) {
        ...
        setFormData({
          ...
          author: blog.author || "Venice Wood Ltd",
          seoTags: blog.seoTags || "",
        });

        // Set image preview from media array
        const featuredMedia = blog.media?.find((m) => m.type === "featured");
        if (featuredMedia?.fileId) {
          const imageUrl = `/api/images/${featuredMedia.fileId}`;
      ...
      showToast("error", "Error", "Failed to fetch blog data");
    } finally {
  ...
  const confirmRemoveImage = async () => {
    ...
    // Delete from GridFS if new fileId exists (not original)
    if (fileIdToDelete && !fileIdToDelete.startsWith("http")) {
      try {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // If we have a new uploaded image fileId, include it
      if (formData.featuredImage) {
        submitData.append("featuredImageFileId", formData.featuredImage);
      }

      // Flag to indicate if image should be removed
      ...
      if (result.success) {
        ...
        throw new Error(
          result.error || result.message || "Failed to update blog post",
        );
      }
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-[#fcfaf6] py-12 px-4">
      ...
      <ConfirmModal
        ...
        message="Are you sure you want to remove this image? This action cannot be undone after saving."
        confirmText="Remove"
        type="danger"
      />
